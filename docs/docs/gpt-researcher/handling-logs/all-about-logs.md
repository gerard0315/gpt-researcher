...
You can find the main downloadable log file in the `outputs` folder.  
Detailed per-research logs are also saved under `outputs/research_logs/<task_id>/`.
...
27. **`planning_search_results`**:
* Captures the planning-stage search query and preview results used before subquery generation.
* `metadata`: Includes the query, retriever, result count, and result previews.
28. **`planner_generated_queries`**:
* Shows the exact sub-query candidates generated by the planner.
* `metadata`: Includes all generated sub-queries.
29. **`retriever_search_trace`**:
* Shows per-retriever search execution for each sub-query with preview snippets.
* `metadata`: Includes retriever name, result count, and top result previews.
30. **`subquery_url_selection_trace`**:
* Shows which URLs were selected for scraping for a sub-query.
* `metadata`: Includes selected URL list and count.
31. **`subquery_answer_trace`**:
* Captures an answer/context preview for each sub-query.
* `metadata`: Includes answer preview text and context stats.
...
Per-research runtime traces are also saved in `outputs/research_logs/<task_id>/` as:
* `events.json`: Structured snapshot of the full run
* `events.jsonl`: Append-only event stream for line-by-line analysis
